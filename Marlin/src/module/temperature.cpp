 ...
 */
  ...
  void Temperature::PID_autotune(const celsius_t target, const heater_id_t heater_id, const int8_t ncycles, const bool set_result/*=false*/) {
    ...
    TERN_(DWIN_LCD_PROUI, dwinPIDTuning(TERN_(PIDTEMPCHAMBER, ischamber ? PIDTEMPCHAMBER_START :) TERN_(PIDTEMPBED, isbed ? PIDTEMPBED_START :) TERN(PIDTEMP, PIDTEMP_START, AUTOTUNE_DONE)));
